...
interface StoreState{
    ...
    totalItems: number,
    isLoading: boolean,
    asyncTableData: (url: string) => void
}
...
const useStore = create<StoreState>((set) => {
    return {
        ...
        totalItems: 0,
        isLoading: true,
        asyncTableData: async (url: string) => {
            axios.get(url)
                .then(response => {
                    console.log(response.data); // 从响应中获取数据
                    const {data: photosData, pagination} = response.data;
                    set((state) => ({tableData: photosData, totalItems: pagination.totalItems, isLoading: false}))
                })
                .catch(error => {
                    console.error(error); // 处理错误
                    // set((state) => ({tableData: [], totalItems: 0, isLoading: false}))
                    set({tableData: [], totalItems: 0, isLoading: false})
                });
        }
...
const ZustandTable: React.FC = () => {
    const {tableData, totalItems, isLoading, asyncTableData} = useStore()
