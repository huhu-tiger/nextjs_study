...
import { LoginVerify, LoginVerifyRes } from '@/types/next-dev'
...
export default function BlogPage() {
  ...
  useEffect(() => {
    const { token,login , timestamp } = cookiesfn()
    console.log('login:', login)
    console.log('timestamp:', timestamp)
    console.log('token:', token)

    async function verify() {
      const verifyData: LoginVerify = { token: token as string, login: login as string, timestamp: Number(timestamp) }
      ...
      const resData: LoginVerifyRes = await response.json()
      console.log('resData:', resData)
      if (resData.valid) {
        message.success('Cookie 验证成功')
      } else {
        message.error('Cookie 验证失败')
      }
    }
